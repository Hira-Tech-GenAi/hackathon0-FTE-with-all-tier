...
for directory in [VAULT_DIR, INBOX_DIR, NEEDS_ACTION_DIR, DONE_DIR, PLANS_DIR, 
                  PENDING_APPROVAL_DIR, APPROVED_DIR, REJECTED_DIR, LOGS_DIR]:
...
def create_plan(objective: str, file_path: str, steps: list = None) -> Dict[str, Any]:
    """
    Create a Plan.md file with objective and action steps.
    
    This skill implements the Claude reasoning loop by creating structured plans
    with checkboxes for tracking progress.
    
    Args:
        objective: The main objective to achieve
        file_path: Path to the trigger file
        steps: Optional list of initial steps (if None, AI will generate)
    
    Returns:
        Dictionary with plan details
    
    Example:
        >>> create_plan("Send invoice to client", "Needs_Action/invoice_request.txt")
        {"success": True, "plan_path": "Plans/PLAN_invoice_client.md", ...}
    """
    try:
        timestamp = datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ")
        plan_id = f"PLAN_{Path(file_path).stem}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
"""
        ...
        write_result = write_to_file(str(plan_path), plan_content)
        
        if not write_result["success"]:
            return {
                "success": False,
                "error": write_result.get("error", "Failed to write plan")
            }
        ...
        return {
            ...
            "objective": objective,
            "total_steps": len(steps),
            "message": f"Plan created: {plan_filename}"
        }
...
def request_approval(action_type: str, details: Dict[str, Any], 
                     reason: str = "") -> Dict[str, Any]:
    """
    Create an approval request file for human-in-the-loop workflow.
    
    This skill implements the HITL pattern by creating approval request files
    that must be moved to /Approved or /Rejected by a human.
    
    Args:
        action_type: Type of action requiring approval (e.g., "payment", "email_send")
    ...
    Example:
        >>> request_approval("payment", {"amount": 500, "to": "Client A"}, "Invoice payment")
        {"success": True, "approval_path": "Pending_Approval/PAYMENT_...", ...}
    """
    try:
        timestamp = datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ")
        expires = (datetime.now().replace(hour=23, minute=59, second=59)).strftime("%Y-%m-%dT%H:%M:%SZ")
        
        # Create safe filename
        safe_type = action_type.upper().replace(" ", "_")[:20]
        approval_id = f"{safe_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
**Created:** {timestamp}
**Reason:** {reason or "Sensitive action requires human approval"}
...
The system will automatically execute the action once approved.
...
### Auto-Expiry
This request will auto-expire at {expires} if not actioned.
...
"""
        ...
        write_result = write_to_file(str(approval_path), approval_content)
        
        if not write_result["success"]:
            return {
                "success": False,
                "error": write_result.get("error", "Failed to create approval request")
            }
        
        # Update dashboard
        update_dashboard(
            status="Waiting Approval",
            action=f"Approval requested: {action_type}",
            file_name=approval_filename,
            human_attention=True,
            human_attention_reason=f"Approval needed for {action_type}"
        )
        ...
        return {
            ...
            "action_type": action_type,
            "status": "pending",
            "message": f"Approval request created: {approval_filename}"
        }
...
def check_approval_status(approval_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        pending_path = PENDING_APPROVAL_DIR / f"{approval_id}.md"
        ...
        # Check in Approved
        approved_path = APPROVED_DIR / f"{approval_id}.md"
        ...
        # Check in Rejected
        rejected_path = REJECTED_DIR / f"{approval_id}.md"
...
def log_action(action_type: str, details: Dict[str, Any], 
               result: str = "success") -> Dict[str, Any]:
    """
    Log an action to the audit log.
    
    This implements the security requirement for audit logging.
    ...
    try:
        ...
        log_entry = {
            ...
            "details": details,
            "result": result,
            "approval_status": "auto" if result == "success" else "N/A"
        }
...
def send_email_draft(to: str, subject: str, body: str, 
                     requires_approval: bool = True) -> Dict[str, Any]:
    """
    Draft or send an email (with optional approval workflow).
    
    This is a Silver Tier MCP-style skill for email actions.
...
status: {"pending_approval" if requires_approval else "ready_to_send"}
---
...
"""
            ...
            email_content += """
This email is ready to send via MCP server.
"""
        
        # Save email draft
        safe_subject = "".join(c if c.isalnum() or c in " -_" else "_" for c in subject[:30])
        email_filename = f"EMAIL_{safe_subject}_{timestamp}.md"
        email_path = INBOX_DIR / email_filename
        
        write_result = write_to_file(str(email_path), email_content)
        
        if not write_result["success"]:
            return {
                "success": False,
                "error": write_result.get("error", "Failed to create email draft")
            }
        
        # Log the action
        log_action("email_draft", {
            "to": to,
            "subject": subject,
        ...
        return {
            ...
            "status": "pending_approval" if requires_approval else "ready_to_send",
            "message": f"Email draft created: {email_filename}"
        }
...
def update_plan_step(plan_path: str, step_number: int, 
                     completed: bool = True) -> Dict[str, Any]:
    """
    Update a step in a plan file.
    ...
    try:
        # Read the plan
        read_result = read_file(plan_path)
        
        if not read_result["success"]:
            return {
                "success": False,
                "error": read_result.get("error", "Could not read plan")
            }
        ...
        new_content = "\n".join(lines)
        write_result = write_to_file(plan_path, new_content)
        
        return {
            "success": write_result["success"],
            "step_number": step_number,
            "completed": completed,
            "message": f"Step {step_number} {'completed' if completed else 'uncompleted'}"
        }
